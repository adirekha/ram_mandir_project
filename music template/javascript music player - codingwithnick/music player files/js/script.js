...
mainAudio.addEventListener("timeupdate", (e) => {
    ...
    mainAudio.addEventListener("loadeddata", () => {
        ...
        let totalSec = Math.floor(mainAdDuration % 60);  if (totalSec < 10) { //if sec is less than 10 then add 0 before it
            totalSec = `0${totalSec}`;
        }
...
// let create li tags according to array lenght for listfor (let i = 0; i < allMusic.length; i++) {
  ...
  liAudioTag.addEventListener("loadeddata", () => {
    ...
    let totalSec = Math.floor(duration % 60); if (totalSec < 10) { //if sec is less than 10 then add 0 before it
        totalSec = `0${totalSec}`;
    }
...
function playingSong() {
   ...
   let audioTag = allLiTags[j].querySelector(".audio-duration");
        ...
        if(allLiTags[j].classList.contains("playing")){
            allLiTags[j].classList.remove("playing");  //  let's get that audio duration value and pass to .audio-duration innertext
